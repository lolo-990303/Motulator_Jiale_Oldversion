...
class VectorCtrl:
    ...
    def __call__(self, w_m_ref, w_M, theta_M, i_s_abc, u_dc):
        ...
        # Space vector and cotau_Mordinate transformation
        ...
        # Data logging
        self.datalog.save([i_s_ref, i_s, u_s_ref, np.nan, w_m_ref, w_m, theta_m,
...
class Datalogger:
    ...
    def plot(self, mdl, base):
        ...
        ax1.legend([r'$\omega_\mathrm{m,ref}$',
                    ...
                    r'$\omega_\mathrm{m}$'],fontsize=18,loc=1)
        ax1.set_xlim(t_range)
        ax1.set_xticklabels([])
        ax1.set_ylabel('Speed (p.u.)',fontsize=20)
        ...
        ax2.legend([r'$\tau_\mathrm{L}$', r'$\tau_\mathrm{M}$',
                    r'$\tau_\mathrm{M,ref}$'],fontsize=18,loc=1)
        ax2.set_ylabel('Torque (p.u.)',fontsize=20)
        ax2.set_xticklabels([])
        ...
        ax3.set_ylabel('Current (p.u.)',fontsize=20)
        ax3.legend([r'$i_\mathrm{sd,ref}$', r'$i_\mathrm{sd}$',
                    r'$i_\mathrm{sq,ref}$', r'$i_\mathrm{sq}$'],fontsize=18,loc=1)
        ...
        ax4.set_ylabel('Voltage (p.u.)',fontsize=20)
        ax4.set_xlim(t_range)
        ax4.set_ylim(0, 1.5)
        ax4.legend([r'$u_\mathrm{s}$', r'$u_\mathrm{dc}/\sqrt{3}$'],fontsize=18,loc=1)
        ax4.set_xticklabels([])

        ax5.plot(data.t, np.abs(data.psi_s)/base.psi)
        ax5.step(self.t, np.abs(self.psi_s)/base.psi, '--', where='post')
        ax5.set_xlim(t_range)
        ax5.legend([r'$\psi_\mathrm{s}$', r'$\hat\psi_\mathrm{s}$'],fontsize=18,loc=1)
        ax5.set_ylabel('Flux (p.u.)',fontsize=20)
    ...
    def plot_latex(self, mdl, base):
        ...
        ax1.plot(data.t, data.w_m/base.w)
        ax1.legend([r'$\omega_\mathrm{m,ref}$',
                    r'$\omega_\mathrm{m}$'],fontsize=18)
        ax1.set_xlim(t_range)
        ax1.set_ylim(-1.2, 1.2)
        ax1.set_xticklabels([])
        ax1.set_ylabel('Speed (p.u.)',fontsize=18)

        ax2.plot(data.t, data.tau_L/base.tau, '--')
        ax2.plot(data.t, data.tau_M/base.tau)
        ax2.set_xlim(t_range)
        ax2.set_ylim(-.2, 1.5)
        ax2.legend([r'$\tau_\mathrm{L}$', r'$\tau_\mathrm{M}$'],fontsize=18)
        ax2.set_ylabel('Torque (p.u.)',fontsize=18)
        ax2.set_xticklabels([])

        ax3.step(self.t, self.i_s.real/base.i, where='post')
        ax3.step(self.t, self.i_s.imag/base.i, where='post')
        ax3.set_ylabel('Current (p.u.)',fontsize=18)
        ax3.legend([r'$i_\mathrm{sd}$',  r'$i_\mathrm{sq}$'],fontsize=18)
        ax3.set_xlim(t_range)
        ...
        ax4.step(self.t, np.abs(self.u_s)/base.u, where='post')
        ax4.step(self.t, self.u_dc/np.sqrt(3)/base.u, '--', where='post')
        ax4.set_ylabel('Voltage (p.u.)',fontsize=18)
        ax4.set_xlim(t_range)
        ax4.set_ylim(0, 1.2)
        ax4.legend([r'$u_\mathrm{s}$', r'$u_\mathrm{dc}/\sqrt{3}$'],fontsize=18)
        ax4.set_xticklabels([])
        ...
        ax5.set_xlim(t_range)
        ax5.set_ylim(0, 1.2)
        ax5.legend([r'$\psi_\mathrm{s}$', r'$\hat\psi_\mathrm{s}$'],fontsize=18)
        ax5.set_ylabel('Flux (p.u.)',fontsize=18)
    ...
    def plot_extra(self, mdl, base):
        ...
        if mdl.pwm is not None:
            ...
            ax1.plot(data.t, data.u_ss.real)# q*udc
            ax1.plot(self.t, self.u_ss.real)# U_ref
            # print('amplitude of reference volatge is:')
            # print(np.max(self.u_ss.real))
            ax1.set_xlim(t_zoom)
            # ax1.set_ylim(-1.5, 1.5)
            ax1.legend([r'$u_\mathrm{sa}$', r'$\hat u_\mathrm{sa}$'],fontsize=15)
            ...
            ax2.legend([r'$i_\mathrm{sa}$', r'$i_\mathrm{sb}$',
                        r'$i_\mathrm{sc}$'],fontsize=15)
            ax2.set_ylabel('Current (p.u.)',fontsize=18)
    ...
    def plot_neutralpointvoltage(self, mdl):
        plt.step(self.t,mdl.converter.delta_ucs)
        plt.xlabel('Time (s)',fontsize=16)
        plt.ylabel(r'$\Delta u_c(V)$',fontsize=16)
        plt.show()
